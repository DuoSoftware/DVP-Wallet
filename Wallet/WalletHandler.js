      ...
      DbConn.Wallet.find({
        ...
        .then(function (wallet) {
          if (wallet) {
            ...
            directPayment.BuyCredit(wallet, amount).then(
              function (charge) {
                var credit = parseFloat(wallet.Credit) + amount;
                DbConn.Wallet.update(
                  {
                    Credit: credit,
                  },
                  {
                  ...
                  .then(function (cmp) {
                    ...
                    if (cmp[0] === 1) {
                      ...
                      deferred.reject(
                        new Error(
                          "Fail to Update Wallet. Please Contact System Administrator."
                        )
                      );
                    }
                    var data = {
          ...
          .then(function (cmp) {
            ...
            var data = {
              ...
              Reason: reqData.Reason
                ? reqData.Reason
                : "Deduct Credit using Credit Card",
              SessionID: reqData.SessionID,
              TxnType: reqData.TxnType,
            };
            data.Credit =
              credit + (wallet.LockCredit ? parseFloat(wallet.LockCredit) : 0);
            addHistory(data);
          })
  ...
  DbConn.Wallet.find({
    ...
    .then(function (wallet) {
      if (wallet) {
        ...
        if (credit >= amount) {
          deductCredit(reqData, wallet, credit, amount).then(
            function (cmp) {
              ...
              if (wallet.AutoRecharge && cmp < wallet.ThresholdValue) {
                var amount = Math.ceil(
                  parseFloat(wallet.AutoRechargeAmount - cmp)
                );
                buyCredit(
                  wallet.WalletId,
                  amount,
            ...
            function (error) {
              ...
              logger.error(
                "DeductCredit - Fail To Update Wallet. - [%s] .",
                jsonString
              );
              callback(jsonString);
            }
          ...
          if (wallet.AutoRecharge) {
            buyCredit(
                ...
                if (cmp > amount) {
                  ...
                  deductCredit(reqData, wallet, credit, amount).then(
                    function (cmp) {
                      ...
                      logger.info(
                        "DeductCredit-buyCredit - Update Wallet - [%s] .",
                        jsonString
                      );
                      callback(jsonString);
                    },
                    function (error) {
                      ...
                      logger.error(
                        "DeductCredit-buyCredit - Fail To Update Wallet. - [%s] .",
                        jsonString
                      );
                      callback(jsonString);
                    }
                  ...
                  var jsonString = messageFormatter.FormatMessage(
                    new Error("Insufficient  Credit Balance."),
                    "EXCEPTION",
                    false,
                    undefined
                  );
              ...
              function (error) {
                ...
                logger.error(
                  "BuyCredit - Fail To Update Wallet. - [%s] .",
                  jsonString
                );
                callback(jsonString);
              }
...
var deductCreditFromTemp = function (
  sessionId,
  reason,
  invokeBy,
  tenant,
          ...
          DbConn.Wallet.find({
            ...
            .then(function (wallet) {
              if (wallet) {
                ...
                redlock.lock("lock:" + walletId, ttl).then(function (lock) {
                  var lockCredit =
                    parseFloat(wallet.LockCredit) - parseFloat(amount);
                  if (lockCredit < 0) {
                    deferred.reject(
                      new Error(
                        "Invalid Amount. Please Contact System Administrator."
                      )
                    );
                    return;
                  }
                  ...
                  DbConn.WalletSessionData.update(
                    {
                      LockCredit: 0,
                    },
                    {
...
module.exports.CreatePackage = function (req, res) {
  directPayment.DirectPayment(req.body).then(
    function (customer) {
      DbConn.Wallet.create({
        Owner: req.user.iss,
        ...
        .error(function (err) {
          ...
          logger.error(
            "CreatePackage - Fail To Create Wallet. - [%s] .",
            jsonString
          );
          res.end(jsonString);
        });
    ...
    function (err) {
      ...
      logger.error(
        "CreatePackage-DirectPayment - Fail To Create Wallet. - [%s] .",
        jsonString
      );
      res.end(jsonString);
    }
...
module.exports.CreateWallet = function (req, res) {
  directPayment.CustomerRegister(req.headers["api_key"], req.body).then(
    function (customer) {
      console.log(
        "CustomerRegister............... : " + req.headers["api_key"]
      );
      DbConn.Wallet.create({
        Owner: req.user.iss,
        StripeId: customer.id,
        ...
        .error(function (err) {
          ...
          logger.error(
            "CreateWallet - Fail To Create Wallet. - [%s] .",
            jsonString
          );
          res.end(jsonString);
        });
    ...
    function (err) {
      ...
      logger.error(
        "CreateWallet-DirectPayment - Fail To Create Wallet. - [%s] .",
        jsonString
      );
      res.end(jsonString);
    }
...
module.exports.UpdateWallet = function (req, res) {
  DbConn.Wallet.update(
    ...
    .error(function (err) {
      ...
      logger.error(
        "UpdateWallet - Fail To Update Wallet. - [%s] .",
        jsonString
      );
      res.end(jsonString);
    });
...
module.exports.BuyCredit = function (req, res) {
  buyCredit(req.params.WalletId, req.body.Amount, req.user).then(
    function (cmp) {
      var jsonString = messageFormatter.FormatMessage(
        undefined,
        "EXCEPTION",
  ...
  DbConn.Wallet.find({
    ...
    .then(function (wallet) {
      if (wallet) {
        ...
        var credit = parseFloat(wallet.Credit);
        if (credit > amount) {
          deductCredit(req, wallet, credit, amount).then(
            function (cmp) {
              var jsonString = messageFormatter.FormatMessage(
                undefined,
                "EXCEPTION",
          ...
          if (wallet.AutoRecharge) {
            if (wallet.ThresholdValue > credit) {
              ...
              if (b > 0 && b > amount) {
                buyCredit(wallet.WalletId, b, req.user).then(
                  function (cmp) {
                    if (cmp > amount) {
                      deductCredit(req, wallet, credit, amount).then(
                        function (cmp) {
...
module.exports.AddNewCard = function (req, res) {
  DbConn.Wallet.find({
    ...
    .then(function (wallet) {
      if (wallet) {
        directPayment.AddNewCard(wallet.StripeId, req.headers["api_key"]).then(
          function (customer) {
            var jsonString = messageFormatter.FormatMessage(
              undefined,
              "Add new Card Successfully.",
              true,
              customer.id
            );
...
module.exports.RemoveCard = function (req, res) {
  DbConn.Wallet.find({
    ...
    .then(function (wallet) {
      if (wallet) {
        directPayment.ListCards(wallet.StripeId).then(
          function (cards) {
            if (cards.data && cards.data.length > 1) {
              directPayment.DeleteCard(wallet.StripeId, req.params.CardId).then(
                function (customer) {
                  if (customer.deleted) {
                    var jsonString = messageFormatter.FormatMessage(
                      undefined,
                      "Card Was Removed Form System.",
                      true,
                      customer.id
                    );
                    logger.info(
                      "RemoveCard - Update Wallet - [%s] .",
                      jsonString
                    );
                    res.end(jsonString);
                    var data = {
                    ...
                    var jsonString = messageFormatter.FormatMessage(
                      undefined,
                      "Fail To Remove Card form Wallet",
                      false,
                      undefined
                    );
                    logger.error(
                      "RemoveCard-DirectPayment - Fail To Remove Card form Wallet. - [%s] .",
                      jsonString
                    );
                    res.end(jsonString);
                  }
              ...
              var jsonString = messageFormatter.FormatMessage(
                undefined,
                "Invalid Card Details Or No Card Added. if You want to Delete Default Card, Please Contact System Administrator'.",
                false,
                undefined
              );
              logger.error(
                "RemoveCard - Fail To Get Card Details.. - [%s] .",
                jsonString
              );
              res.end(jsonString);
            }
      ...
      if (wallet) {
        directPayment.ListCards(wallet.StripeId).then(
          ...
          function (err) {
            ...
            logger.error(
              "ListCards - Fail To Get Card Details.. - [%s] .",
              jsonString
            );
            res.end(jsonString);
          }
...
module.exports.SetDefaultCard = function (req, res) {
  DbConn.Wallet.find({
    ...
    .then(function (wallet) {
      if (wallet) {
        directPayment.SetDefaultCard(wallet.StripeId, req.params.CardId).then(
          function (cards) {
            var jsonString = messageFormatter.FormatMessage(
              undefined,
              "Successfully Set Default card",
              true,
              undefined
            );
          ...
          function (err) {
            ...
            logger.error(
              "SetDefaultCard - Fail Set Default Card. - [%s] .",
              jsonString
            );
            res.end(jsonString);
          }
...
var addWalletSessionData = function (data) {
  try {
    DbConn.WalletSessionData.create({
      Credit: data.Credit,
...
var DeleteWalletSessionData = function (data) {
  try {
    var deferred = Q.defer();
    DbConn.WalletSessionData.destroy({
      where: [
        { SessionID: data.SessionID },
        { TenantId: data.TenantId },
        { CompanyId: data.CompanyId },
...
var addHistory = function (data) {
  DbConn.WalletHistory.create({
    ...
    .error(function (err) {
      ...
      logger.error(
        "addHistory - Fail To Create WalletHistory. - [%s] .",
        jsonString
      );
    });
};

var calculateChargeForSession = function (
  sessionId,
  credit,
  tenant,
  company,
          ...
          .then(function (wallet) {
            ...
            if (wallet) {
              redlock.lock("lock:" + sessionId, ttl).then(function (lock) {
                var credit = parseFloat(wallet.Credit);
                var lockCredit = wallet.LockCredit
                  ? parseFloat(wallet.LockCredit)
                  : 0;
                if (credit > amount) {
                  credit = credit - amount;
              ...
              deferred.reject(
                new Error(
                  "Fail to Find Wallet. Please Contact System Administrator."
                )
              );
            }
          })
